"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.EpiDeliveryRepository = void 0;
const dataSource_1 = require("../../../database/dataSource");
const EpiDelivery_1 = require("../../../database/entity/EpiDelivery");
exports.EpiDeliveryRepository = dataSource_1.AppDataSource.getRepository(EpiDelivery_1.EpiDelivery).extend({
    getDeliveryContext(inventoryId) {
        return __awaiter(this, void 0, void 0, function* () {
            // Retorna a lista de Empregados com seus respectivos WorkUnits vinculados
            const query = `
        select 
            e.id as "employeeId",
            e.name as "employeeName",
            w.id as "workUnitId",
            w.name as "workUnitName"
        from work_unit w
        inner join "workUnit_role" wr on wr."workUnit_id" = w.id
        inner join "role" r on r.id = wr.role_id
        inner join "employee_roles_role" er on er."roleId" = r.id
        inner join "employee" e on e.id = er."employeeId"
        where w."inventoryId" = $1;
    `;
            const result = yield dataSource_1.AppDataSource.query(query, [inventoryId]);
            return result;
        });
    },
    getRecommendations(workUnitId) {
        return __awaiter(this, void 0, void 0, function* () {
            // Retorna a lista de EPIs recomendados
            const query = `
        select
            e.id as "epiId",
            e.name as "epiName",
            e."caNumber" as "caNumber"
        from work_unit w
        inner join hazard_assessment h on h."workUnitId" = w.id
        inner join row_factor rf on rf."hazardAssessmentId" = h.id
        inner join control_measure cm on cm.id = rf."controlMeasureId"
        inner join control_measure_epi cme on cme."controlMeasureId" = cm.id
        inner join epi e on e.id = cme."epiId"
        where w.id = $1;
    `;
            const result = yield dataSource_1.AppDataSource.query(query, [workUnitId]);
            return result;
        });
    }
});
